 ...
 <?php
    ...
    if(isset($_POST['submit']))
    {
		   ...
		   $JOINING_DATE=mysqli_real_escape_string($con,trim($_POST['JOINING_DATE']));
            $SQL = "SELECT * FROM $tbl_name WHERE `IS_DELETED` = 0 AND (`EMPLOYEE_ID` = '$EMPLOYEE_ID'   OR `EMPLOYEE_EMAIL` = '$EMPLOYEE_EMAIL' OR `MOBILE_NO` = '$MOBILE_NO' )  ";
        ...
        if ($_FILES["IMAGE_URL"]["error"] > 0) {
            ...
            move_uploaded_file($_FILES["IMAGE_URL"]["tmp_name"],"upload/" . $IMAGE_URL);
              ...
              $sql = "INSERT INTO $tbl_name ( `EMPLOYEE_ID` ,`EMPLOYEE_PASSWORD` , `EMPLOYEE_NAME`,`EMPLOYEE_EMAIL` ,`MOBILE_NO` ,`DESIGNATION_NO` ,`JOINING_DATE`,`ZONE_NO` ,`IS_ACTIVE`,`IMAGE_URL`,
			   `CREATED_BY` , `CREATED_ON`) VALUES(  '$EMPLOYEE_ID', '$EMPLOYEE_PASSWORD', '$EMPLOYEE_NAME','$EMPLOYEE_EMAIL','$MOBILE_NO','$DESIGNATION_NO','$JOINING_DATE','$ZONE_NO',1,'$IMAGE_URL','$user_no', '$CURR_TIME')";
                $result = mysqli_query($con,$sql);
                if($result)
                {
    ...
    if(isset($_POST['update']))
    {
           ...
           $DESIGNATION_NO = trim($_POST['DESIGNATION_NO']);
		   $JOINING_DATE=mysqli_real_escape_string($con,trim($_POST['JOINING_DATE']));
            $SQL = "SELECT * FROM $tbl_name WHERE `IS_DELETED` = 0 AND (`EMPLOYEE_ID` = '$EMPLOYEE_ID'   OR `EMPLOYEE_EMAIL` = '$EMPLOYEE_EMAIL' OR `MOBILE_NO` = '$MOBILE_NO' ) AND `EMPLOYEE_REG_NO` != '$EMPLOYEE_REG_NO' ";
            $COUNT = mysqli_num_rows(mysqli_query($con,$SQL));
            if($COUNT < 1): 
                if ($_FILES["IMAGE_URL"]["error"] > 0) {
            ...
            move_uploaded_file($_FILES["IMAGE_URL"]["tmp_name"],"upload/" . $IMAGE_URL);
                ...
                $sql = "UPDATE $tbl_name SET   `EMPLOYEE_ID` = '$EMPLOYEE_ID' , `EMPLOYEE_NAME` = '$EMPLOYEE_NAME' ,
 ...
 <div class="form-group ">
        ...
        <fieldset class="scheduler-border">
                ...
                <div class="form-group ">
                    ...
                    <div class="col-lg-4">
                        ...
                        <input class=" form-control" id="EMPLOYEE_ID" name="EMPLOYEE_ID" type="text"  style="" >
                ...
                <div class="form-group ">
                    ...
                    <div class="col-lg-4">
                       <select class="form-control search" name="DESIGNATION_NO" id="DESIGNATION_NO" style="width: 100%">
                            <option value="-1">-- Select Designation --</option>
                         <?php
                                $sql = "SELECT * FROM `gen_designations` where IS_DELETED=0 ";
                                $result1 = mysqli_query($con,$sql);
    ...
    if(isset($_POST['searchBtn']))
    {
        ...
        $EMPLOYEE_ID =mysqli_real_escape_string($con,trim($_POST['EMPLOYEE_ID']));
          if($EMPLOYEE_ID != "-1"){
            $where.=" AND `gen_groups`.`EMPLOYEE_ID` LIKE '%$EMPLOYEE_ID%'";
          }
    ...
    */
    ...
    /* Get data. */
       ...
       $sql = "SELECT * FROM $tbl_name  LEFT JOIN `gen_designations` ON `gen_designations`.`DESIGNATION_NO`=$tbl_name.`DESIGNATION_NO` LEFT JOIN `gen_zones` ON `gen_zones`.`ZONE_NO`=$tbl_name.`ZONE_NO` WHERE $tbl_name.`IS_DELETED` = 0 $where ORDER BY `EMPLOYEE_REG_NO` DESC LIMIT $start, $limit";
    $result = mysqli_query($con,$sql);
